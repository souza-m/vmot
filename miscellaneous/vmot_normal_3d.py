...
def sample(n_points, coupling = 'independent', clip_normal = None, fix_x = None, seed = None):
    ...
    if clip_normal is None:   # noisy tails included
        ...
        if coupling == 'exact':
            ...
            if fix_x is None:
                random_sample = rv.rvs(size=[n_points,3])
    ...
    if coupling == 'positive':
        ...
        X2 = np.sort(X2)
        X3 = np.sort(X3)
        X = np.vstack((np.sort(X1), np.sort(X2), np.sort(X3))).T
        np.random.shuffle(X)
        X1, X2, X3 = X[:,0], X[:,1], X[:,2]
    
    X, Y = np.array([X1, X2, X3]).T, np.array([Y1, Y2, Y3]).T
    return X, Y

figsize = [12,12]
def plot_sample(X, Y, label):
    X1, X2, X3, Y1, Y2, Y3 = X[:,0], X[:,1], X[:,2], Y[:,0], Y[:,1], Y[:,2]
    fig, ax = pl.subplots(2,3, sharex = True, sharey = True, figsize=figsize)
    ax[0][0].set_title(label + 'X sample ' + label)
    ax[0][0].set_xlabel('X1')
    ax[0][0].set_ylabel('X2')
    ax[0][0].scatter(X1, X2, alpha=.05)
    ax[0][1].set_title('X sample ' + label)
    ax[0][1].set_xlabel('X1')
    ax[0][1].set_ylabel('X3')
    ax[0][1].scatter(X1, X3, alpha=.05)
    ax[0][2].set_title('X sample ' + label)
    ax[0][2].set_xlabel('X2')
    ax[0][2].set_ylabel('X3')
    ax[0][2].scatter(X2, X3, alpha=.05)
    ax[1][0].set_title('Y sample ' + label)
    ax[1][0].set_xlabel('Y1')
    ax[1][0].set_ylabel('Y2')
    ax[1][0].scatter(Y1, Y2, alpha=.05)
    ax[1][1].set_title('Y sample ' + label)
    ax[1][1].set_xlabel('Y1')
    ax[1][1].set_ylabel('Y3')
    ax[1][1].scatter(Y1, Y3, alpha=.05)
    ax[1][2].set_title('Y sample ' + label)
    ax[1][2].set_xlabel('Y2')
    ax[1][2].set_ylabel('Y3')
    ax[1][2].scatter(Y1, Y3, alpha=.05)
...
if False:
    ...
    plot_sample(sample_th_X, sample_th_Y, 'theta')
...
# --- cost function ---
a12, a13, a23 = 0, 0, 0   # y only
b12, b13, b23 = 1, 2, 0.5
label_cross_product_y    = 'cross_product_y'
def cost_cross_product_y(x, y):
    ...
    y2 = y[:, 1]
    y3 = y[:, 2]
    return a12 * x1 * x2 + a13 * x1 * x3 + a23 * x2 * x3 + \
           b12 * y1 * y2 + b13 * y1 * y3 + b23 * y2 * y3

# penalty function
beta = mmot.beta_L2

# ref value
sig1, sig2, sig3 = x_scale
rho1, rho2, rho3 = y_scale
lamb1 = np.sqrt(rho1 ** 2 - sig1 ** 2)
lamb2 = np.sqrt(rho2 ** 2 - sig2 ** 2)
lamb3 = np.sqrt(rho3 ** 2 - sig3 ** 2)

ref_max = a12 * sig1 * sig2 + a13 * sig1 * sig3 + a23 * sig2 * sig3 + \
          b12 * (sig1 * sig2 + lamb1 * lamb2) + \
          b13 * (sig1 * sig3 + lamb1 * lamb3) + \
          b23 * (sig2 * sig3 + lamb2 * lamb3)
...
_file = 'results_' + primal_obj + '_' + f_label + '_' + coupling + '.pickle'
_path = _dir + _file
...
# load & show
_dir = 'U:/Projects/MMOT/'
labels = ['results_max_cross_product_independent',
          'results_max_cross_product_positive' ]
...
for file in files:    
    ...
    value_series   = results['value_series'][:1520]
...
for file in files:    
    ...
    std_series     = results['std_series'][:1520]
    pl.fill_between(range(len(value_series)), value_series + std_series, value_series - std_series, alpha = .5, facecolor = 'grey')
