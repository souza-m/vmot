...
for T in [2, 3]:
    for monotone in [False, True]:
        ...
        if T == 2:
            ...
            density_set.append([x.copy(), y.copy(), pi_hat.copy(), cut, umargin])
            ...
            pi_hat /= pi_hat.sum()
            cut = np.quantile(pi_hat, 1 - (5000 / n_points))
            density_set.append([x.copy(), y.copy(), z.copy(), pi_hat.copy(), cut, umargin])
...
[(d[-1], 0.) for d in density_set]
tolerance_values = [(d[-1], 0.) for d in density_set]
# [
#     (0.016414525985717774, 0.),  # First line
#     (0.014381345748901368, 0.),  # Second line
#     (0.019051630020141604, 0.),  # Third line
#     (0.01609282684326172, 0.)   # Fourth line
# ]

cc = ['#1965B0', '#DC050C', '#F1932D', '#4EB265', '#E7D046', '#F6C141']
...
for i in range(2):
    ...
    line, = ax[0].plot(_range, values, color=cc[i], label=labels[i])  # Store handle
    ax[0].scatter(_range, values, color=cc[i], s=5)
    ax[0].fill_between(_range, lower_tol, upper_tol, color=cc[i], alpha=0.2)  # Tolerance band
    line_handles.append(line)  # Save line for legend

ax[0].axhline(OT_value, linestyle=':', linewidth=1.5, color='black', label="OT")
ax[0].axhline(VMOT_value, linestyle='--', linewidth=1.5, color='black', label="VMOT")
line_handles.append(ax[0].lines[-2])  # Add OT line
line_handles.append(ax[0].lines[-1])  # Add VMOT line

ax[0].legend(handles=line_handles, labels=labels + ["OT", "VMOT"], loc='center right', bbox_to_anchor=(1., 0.7, 0.0, 0.0))
# ax[0].scatter(_range, value_series[1][tail_cut:], s=5)
ax[0].set_xlabel('Iteration')
ax[0].set_title(title_T2)

# Reset legend handles for second plot
line_handles = []
...
for i in range(2, 4):
    ...
    line, = ax[1].plot(_range, values, color=cc[i-2], label=labels[i])  # Store handle
    ax[1].scatter(_range, values, color=cc[i-2], s=5)
    ax[1].fill_between(_range, lower_tol, upper_tol, color=cc[i-2], alpha=0.2)  # Tolerance band
    line_handles.append(line)  # Save line for legend

ax[1].axhline(OT_value, linestyle=':', linewidth=1.5, color='black', label="OT")
line_handles.append(ax[1].lines[-1])  # Add OT line

ax[1].legend(handles=line_handles, labels=labels[:-1] + ["OT"], loc='best', bbox_to_anchor=(1., 0.8, 0.0, 0.0))
